...
###############
@cython.cfunc
def get_fixed_point_case1(
    order: cython.double, upper: cython.double, tol: cython.double = 1e-7
):
    """
...
@cython.cfunc
def _core_logic_case1(
    alpha: cython.double,
    upper: cython.double,
    order: cython.double,
    one_minus_order: cython.double,
    ...
    high: cython.double = one_minus_alpha
    ...
    M_low: cython.double = c2 if alpha <= fixed_point else INFINITY

    # M_high: cython.double = c1 * pow(high, expo) + c2 * pow(1 - high, expo)
    M_high: cython.double = c1 * pow(high, expo) + c2 * pow(alpha, expo)
...
@cython.cfunc
def get_FNR_case1_vec(
    alpha_array: cython.double[:],
    order: cython.double,
    ...
    log_upper: cython.double = (order - 1.0) * epsilon
    upper: cython.double = exp(log_upper)
    one_minus_order: cython.double = 1.0 - order

    # compute fixed point (hard code 1e-15 for stability)
    fixed_point: cython.double = get_fixed_point_case1(order, upper, tol=1e-15)

    # allocate output array
    n: cython.Py_ssize_t = alpha_array.shape[0]
    ...
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case1(
...
###############
@cython.cfunc
def get_fixed_point_case2(epsilon: cython.double, tol: cython.double = 1e-7):
    """
...
def _core_logic_case2(
    ...
    # save variables to reduce computation of M for later
    one_minus_alpha: cython.double = 1.0 - alpha
    log_one_minus_alpha: cython.double = log1p(-alpha)
    log_alpha: cython.double = log(alpha)

    less_than_fixed_point: cython.bint = alpha <= fixed_point
...
@cython.cfunc
def get_FNR_case2_vec(
    alpha_array: cython.double[:], epsilon: cython.double, tol: cython.double = 1e-7
):

    # compute fixed point (hard code 1e-15 tol for stability)
    fixed_point: cython.double = get_fixed_point_case2(epsilon, tol=1e-15)
    ...
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case2(alpha_array[i], epsilon, fixed_point, tol)
...
###############
###############
def get_fixed_point_case3(
    order: cython.double,
    lower: cython.double,
    alpha_max: cython.double,
    tol: cython.double = 1e-7,
):
    """
    See _core_logic_case1 and get_fixed_point_case1 doc string for context on
...
def get_FNR_case3_vec(
    ...
    log_lower: cython.double = (order - 1.0) * epsilon
    lower: cython.double = exp(log_lower)
    one_minus_order: cython.double = 1.0 - order
    alpha_max: cython.double = exp((order - 1) / order * epsilon)

    # compute fixed point (hard code 1e-15 for stability)
    fixed_point: cython.double = get_fixed_point_case3(
        order, lower, alpha_max, tol=1e-15
    )
    ...
    i: cython.Py_ssize_t
    for i in range(n):
        result_view[i] = _core_logic_case3(
